    ...
    def preCompute(self, x, y, data, tick,rng):
        ...
        for dx, dy in neighborhood:
            if rx + dx >= 0 and ry + dy >= 0 and ry + dy < data.nrows and rx + dx < data.ncols:
                ...
                thetaS = np.arctan((data.elevation[ry + dy][rx + dx] - data.elevation[ry][rx]) / (np.sqrt(
                ...
                # Get rothermel values
                # Rs = rothermelRate(thetaS, ang, data.get_windA(tick, x, y), data.get_windV(tick, x, y), h, delta, beta, Mx, w0, sigma)
            ...
            # spotAngOptionsY2 = [-4, -3, -2, -1, 1, 2, 3,
            #                     4]  # in 9x9 grid, relative to center tile, if 0 and 0 then pull Y from this
            ...
            #     rthetawfSpot = abs(math.radians(thetawfSpot))
            #
            ...
            #
            #
            #         pburnSimSpot = int.from_bytes(os.urandom(8), byteorder="big") / ((1 << 64) - 1)
            #
            #         if (Pc >= pburnSimSpot):
            #             print("ok")
            #             Rspot = ((rothermelRateSpot(0, 0, hSpot, deltaSpot, betaSpot, MxSpot, w0Spot,
            #                                         sigmaSpot)) / data.p)
            #             # unit conversion
            #             # print("R val", R)
            #             tsRSpot = (1 / Rspot)
